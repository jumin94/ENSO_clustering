...
year_num = 30
data_file_path = '/home/julia/Desktop/ENSO_flavors_WAF/stationarity_test/data/sst.mnmean_ERSST_2022_KAPLAN_grid.nc'
output_file_path = '/home/julia/Desktop/ENSO_flavors_WAF/stationarity_test/ERSST/'+str(year_num)+'_year_noise_classification_dictionary_ERSST.csv' 
...
def worker2(process):  
    ...
    for kk in range(nr):  #running over each sample
        ...
        print('Iteration',iteration,' of ',nr)
        output = apply_operation_in_parallel(art_sst[iteration],nr,20)
...
for i, result in enumerate(art_sst):
    art_sst_dict[f"Worker {i}"] = result
...
with multiprocessing.Pool(processes=2) as pool:
    art_class = pool.map(worker2, range(2))
    
art_class_dict = {}
for i, result in enumerate(art_class):
    art_class_dict[f"Worker {i}"] = result
...
for jj in range(8):
    ...
    for i, result in enumerate(art_class):
        count =+1
        print('saving ',count)
        art_class_dict[f"Worker {count}"] = result
